...
const NavButton = ({onClick, className, navbarCollapsed}: Props) => {
    const classes = `bg-black dark:bg-white h-0.5 w-7 duration-200 ${navbarCollapsed ? 'absolute':''}`;
  return (
    <button
    className={`${className} w-7 h-7 group transition focus:outline-none `}
    onClick={onClick}
    >
    <div className={`space-y-1.5 flex flex-col items-end relative ${navbarCollapsed ? 'space-y-0 rotate-90 duration-300 delay-100':
